...
| Field | Purpose |
| --- | --- |
| `release.stableVersion` | Version badge on Download CTAs (e.g. `v1.0.12.1`) |
...
The website defaults to branch `feature/v1.0.12.1` for app docs and this config (override with `VITE_SCANLY_GITHUB_REF`). After merge to `master`, point the site env/default at `master`.

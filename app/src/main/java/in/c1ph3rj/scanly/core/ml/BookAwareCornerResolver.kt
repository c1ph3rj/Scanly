...
class BookAwareCornerResolver @Inject constructor(
    ...
    suspend fun detect(bitmap: Bitmap, selectedModel: DocumentCornerModel): CornerResolution {
        val startNanos = System.nanoTime()
        val resolution = resolve(bitmap, selectedModel)
    ...
    private suspend fun resolve(bitmap: Bitmap, selectedModel: DocumentCornerModel): CornerResolution {
        val primaryRaw = runCatching { detector.detect(bitmap, selectedModel) }
            ...
            .getOrThrow()
        val primary = prepareCandidate(bitmap, primaryRaw)
        val needsVerification = primary.refinement?.kind?.let { it != BookPageRefinementKind.NONE } == true ||
            ...
            CornerCandidatePolicy.needsAccurateVerification(primary.result)
        ...
        val accurate = prepareCandidate(bitmap, accurateRaw)
        val combinedTiming = primaryRaw.timing + accurateRaw.timing
        val chosen = choose(primary, accurate)
        return chosen.copy(
    ...
    private fun prepareCandidate(bitmap: Bitmap, raw: CornerDetectionResult): Candidate {
        val readyQuad = raw.quad?.takeIf(DocumentQuadPolicy::isCaptureReady)
            ?: return Candidate(raw.copy(quad = null), null)
        val refinement = BookPageQuadAnalyzer.analyze(bitmap, readyQuad)
        return Candidate(
            result = raw.copy(quad = refinement.quad?.takeIf(DocumentQuadPolicy::isCaptureReady)),
            refinement = refinement,
        )
    }

    private fun choose(primary: Candidate, accurate: Candidate): CornerResolution {
        ...
        if (primaryQuad == null && accurateQuad == null) {
            ...
            ) CornerResolutionIssue.TWO_PAGES_AMBIGUOUS else null
        ...
        val candidatesDisagree = primaryQuad.maxCornerDistance(accurateQuad) >= MODEL_DISAGREEMENT_DISTANCE
        if (candidatesDisagree && abs(primaryScore - accurateScore) < MODEL_DISAGREEMENT_SCORE_MARGIN) {
            return CornerResolution(primary.result.copy(quad = null), CornerResolutionIssue.MODELS_DISAGREE)
        }
        return CornerResolution(if (accurateScore >= primaryScore + ACCURATE_REPLACEMENT_MARGIN) accurate.result else primary.result)
    }

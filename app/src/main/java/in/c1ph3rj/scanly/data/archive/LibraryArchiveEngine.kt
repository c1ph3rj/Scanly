...
class LibraryArchiveEngine @Inject constructor(
    ...
    suspend fun createBackup(progress: ArchiveProgressCallback): String =
        operationCoordinator.withMaintenance("Library backup is in progress.") {
            ...
            val fileName = "Scanly-library-${LocalDateTime.now().format(BACKUP_TIME_FORMAT)}.scanly"
    ...
    suspend fun restore(
        ...
        operationCoordinator.withMaintenance("Library restore is in progress.") {
            ...
            val stagingDirectory = File(context.cacheDir, "$ARCHIVE_WORK_DIRECTORY/restore-$workId")
    ...
    private suspend fun recoverInterruptedRestores() {
        val journalDirectory = File(context.filesDir, ARCHIVE_JOURNAL_DIRECTORY)
    ...
    private fun restorePath(path: String?, oldDocumentId: String, newRoot: File): String? {
        if (path == null) return null
    ...
    private fun readCurrentEntry(input: InputStream, maxBytes: Int): ByteArray {
    ...
    companion object {
        private const val SIGNATURE = "SCANLY_LIBRARY_BACKUP"
        private const val ARCHIVE_VERSION = 1
        private const val MANIFEST_ENTRY = "manifest.json"
        private const val ARCHIVE_WORK_DIRECTORY = "library-archive"
        private const val ARCHIVE_JOURNAL_DIRECTORY = "library-archive-journal"
        private const val SHA_256 = "SHA-256"
        private const val BUFFER_SIZE = 64 * 1024
        private const val MAX_MANIFEST_BYTES = 10 * 1024 * 1024
        private const val MAX_ASSET_COUNT = 100_000
        private val BACKUP_TIME_FORMAT = DateTimeFormatter.ofPattern("yyyy-MM-dd-HHmm")
    }
    ...
    private data class ArchiveManifest(
        ...
        fun validate() {
            check(assets.size <= MAX_ASSET_COUNT) { "Backup contains too many files." }
        ...
        companion object {
            fun fromJson(json: JSONObject): ArchiveManifest {
                check(json.optString("signature") == SIGNATURE) { "This is not a Scanly backup." }
                check(json.optInt("archiveVersion", -1) == ARCHIVE_VERSION) {
    ...
    private data class ArchiveAssetRecord(
        val archivePath: String,
        val size: Long,
        val sha256: String,
    ) {
    ...
    private class RestoreJournal private constructor(
        ...
        companion object {
            fun create(
                ...
                file = File(context.filesDir, "$ARCHIVE_JOURNAL_DIRECTORY/restore-journal-$workId.json"),
                oldRoots = oldRoots,
                newRoots = newRoots,
